...
test("classic cards use a compact fan-safe horizontal index", () => {
  ...
  assert.match(theme, /--face-card:"New York",ui-serif,"Iowan Old Style",Georgia,serif/);
  assert.match(theme, /--face-label:"Marcellus",var\(--serif\)/);
  assert.match(theme, /--type-card-index-suit:1\.12em/);
  ...
  assert.match(html, /\.ix\{font-size:3\.2em\}/);
  assert.doesNotMatch(html, /\.ix\.ten\{font-size:/);
  assert.match(html, /\.ix\.ten i\{letter-spacing:-\.04em\}/);
  assert.doesNotMatch(html, /\.ix\.ten i\{[^}]*transform:scaleX/);
  assert.match(html, /font-size:var\(--type-card-index-suit\)/);
  assert.match(html, /\.ix b span\{\s*display:inline-block;\s*transform:scale\(var\(--suit-optical-scale,1\.18\)\)/);
  assert.match(html, /inline-size:\.72em;text-align:center/);
  ...
  assert.match(html, /const fanOff = cw\*\.40/);
});
